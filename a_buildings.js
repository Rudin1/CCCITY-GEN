...
import {get_color_b, main_matetrial,outer_90,flag_windows} from './a_tools.js';
...
// 1 wall only 
building_arr_vector2(arr,al,height = null,y_s = 0,type = 2,color = 0xffffff,start_lvl=1,flr=1,fv,fh){
...
// a== dl, b == ul, c == ur, d == dr
window_grid2(a,b,c,d,extrude,y_s,start_lvl,flr,fv,fh,w_h = null,w_v = null){
        ...
        if (dist_hor < w_h || dist_ver < w_v){return;} // window > wall --> exit.
        ...
        if(n_ver_cols==0 || n_hor_cols==0){return;}
        ...
        let temp_h = new THREE.Vector3(0,0,0).add(dir_hor.clone().multiplyScalar(w_h*0.5)) //w_h*0.5
        let temp_half = temp_h.clone().divideScalar(2)
        let temp_v = new THREE.Vector3(0,0,0).add(dir_ver.clone().multiplyScalar(w_v*0.5)) //w_v*0.5
       ...
       if(fv){
        ...
        let dir1 = vec1.clone().subVectors(get_points[1],get_points[0]).normalize().multiplyScalar(w_h/2)
        ...
        add_container(main_box,'win_box','win_box', 0.05,ht,0.05,world_pos,world_quat,0x000000) 
...
main_box.rotation.set(0,0,0)
        ...
        for(let i=y_e-flr/2;i>0;i-=flr){ // by dist // each loop, go down from (up to down)


            

            if(i<y_s + flr/2 || i< start_lvl + flr/2){break;}
...
//&&&&
            for(let j=1;j<get_points.length-1;j+=1){ 
                ...
                // if(Math.random()<0.1)continue
                    ...
                    let col = window_cols[Math.floor((window_cols.length-0.01)*Math.random())] 
                    ...
                    if(fh && j%2==0){continue}
                    if(fv && i%2==0){continue}
                    ...
                    if((!fv && j%2==1) || (fv && j%4==1) ){
                            mesh_containers['win'][0].push([
                                ...
                                w_h,(w_v-temp), //w_v,
                                // w_h,w_v, //w_v,
                                world_pos.x,world_pos.y-temp/2,world_pos.z,
                                world_quat.x,
                                world_quat.y,
                                world_quat.z,
                                world_quat.w,
            ...
            main_plain.rotation.set(0,0,0)
                ...
                if(1){
                    ...
                    if(!flag_large && j%2==0){continue}
                    if(fv && j%2==0){continue}
                    if(fh && i%2==0){continue}

               

                        let ll =   new THREE.Vector3(temp_mh.x + temp_h.x,temp_mh.y + temp_h.y,temp_mh.z + temp_h.z).add(vec3)
                        let uu =   new THREE.Vector3(temp_mh.x + temp_v.x,temp_mh.y + temp_v.y,temp_mh.z + temp_v.z).add(vec3)
                        let rr =   new THREE.Vector3(temp_mh.x - temp_h.x,temp_mh.y - temp_h.y,temp_mh.z - temp_h.z).add(vec3)
                        let dd =   new THREE.Vector3(temp_mh.x - temp_v.x,temp_mh.y - temp_v.y,temp_mh.z - temp_v.z).add(vec3)

                        let dl =   new THREE.Vector3(temp_mh.x + temp_h.x,temp_mh.y - temp_h.y - temp_v.y,temp_mh.z + temp_h.z).add(vec3)
                        let ul =   new THREE.Vector3(temp_mh.x + temp_h.x,temp_mh.y + temp_h.y + temp_v.y,temp_mh.z + temp_h.z).add(vec3)
                        let ur =   new THREE.Vector3(temp_mh.x - temp_h.x,temp_mh.y + temp_h.y + temp_v.y ,temp_mh.z - temp_h.z).add(vec3)
                        let dr =   new THREE.Vector3(temp_mh.x - temp_h.x,temp_mh.y - temp_h.y - temp_v.y,temp_mh.z - temp_h.z).add(vec3)
...
function draw_tube2(v1,v2,d = null,achor = null,b_noise = 0.1){
	...
	add_container(main_box,'win_box','win_box',d,0.05,0.05,world_pos,world_quat,0x000000) 
