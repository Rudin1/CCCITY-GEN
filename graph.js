...
class G {
	constructor(arr = null, alpha_car = 0.2, arr_lanes = null) {
		...
		this.arr = arr;
		this.arr_lanes =
			arr_lanes == null ? new Array(arr.length).fill(0) : arr_lanes; // num lanes of each line
		this.nodes = new Map();
	...
	get_parks() {
		...
		props_map.forEach((park) => {
			...
			let rect_arr = [xmax, zmax, xmin, zmin];
			...
			points_temp.forEach((vec) => {
				...
				for (let k = 0; k < amt; k++) {
					...
					if (g_trails_cars && col === c_ground) {
						...
						c10.r = THREE.MathUtils.clamp(
							c10.r + THREE.MathUtils.randFloat(-0.1, 0.1),
							0,
							1
						);
						c10.g = THREE.MathUtils.clamp(
							c10.g + THREE.MathUtils.randFloat(-0.1, 0.1),
							0,
							1
						);
						c10.b = THREE.MathUtils.clamp(
							c10.b + THREE.MathUtils.randFloat(-0.1, 0.1),
							0,
							1
						);
						col = c10;
			...
			if (flag_create) {
				...
				for (let i = 0; i < grid.length; i++) {
					for (let j = 0; j < grid[0].length; j++) {
						if (grid[i][j] !== undefined && grid[i][j] >= 1) {
							...
							let temp = flag_memories
								? THREE.MathUtils.randInt(0, mc_arr.length - 1)
								: 0; /// * change latter
							...
							if (!map_grps.has(grid[i][j])) {
								...
								let size =
									...
									r < 0.2 ? [3, 7, 0] : r < 0.4 ? [5, 10, 0] : [10, 15, 1];
								// r<0.6 ? [10,15,1]:
								// [15,20,1] ;
								...
								let c1 =
									mc_arr[temp][
										Math.floor((mc_arr[temp].length - 0.01) * Math.random())
									];
				...
				if (type == 1) {
					for (let i = 0; i < amt_tree; i++) {
						...
						let colors = flag_memories
							? mc_arr[noise_col_map.get(temp)]
							: arr_colors[noise_col_map.get(temp)]; //['#dfd8cf','#1c1c1c'] //['#654d49','#d5ccc3','#9f948c','#c29487','#9c635b','#292423','#b8acac','#601616','#837c7c','#d1b49c']
	...
	get_sidewalk2() {
		...
		this.poly_graph.forEach((info_poly) => {
			let sidewalk = OffsetContour(
				info_poly.alpha_off / 2,
				info_poly.new_area,
				true
			); ///oof//2

			let building_area = OffsetContour(
				info_poly.alpha_off,
				info_poly.new_area,
				true
			);
	...
	get_verteces() {
		...
		arr.forEach((adj, node) => {
			...
			method = Math.random() > 0.5 ? "edge_vcir" : "edge_roads"; // ??
			...
			if (
				...
				v_mid.z >= s_v - 2 &&
				v_mid.z <= e_v + 2 &&
				(Math.abs(v_mid.x - s_h) < 2 ||
					Math.abs(v_mid.x - e_h) < 2 ||
					Math.abs(v_mid.z - s_v) < 2 ||
					Math.abs(v_mid.z - e_v) < 2)
			) {
			...
			if (method == "edge_roads") {
				// reduce size to avoid car overlapping area
	...
	connect_roads() {
			...
			v.forEach((info, key) => {
				...
				if (info[3] == false) {
					...
					for (let i = 0; i < info_out.length; i++) {
						if (
							node_vec.x === info_out[i][0].x &&
							node_vec.z === info_out[i][0].z
						) {
					...
					if (this.edge_graph.get(edge) !== undefined) {
						...
						if (poly1 !== undefined && poly2 !== undefined) {
							if (poly1.kind == 1 && poly2.kind == 1) {
								// between 2 parks == no road
								is_park = true;
							} else {
								if (poly1.kind == 1 || poly2.kind == 1) {
									// 1 of them is park == edge park --> save edge in park id
		...
		this.poly_graph.forEach((info, name) => {
			...
			if (info.kind == 1) {
				info.poly.forEach((v) => {
					if (
						park_vertecs[1].distanceTo(v) <
						park_vertecs[1].distanceTo(save_avgs[1])
					) {
						poly_edges[1] = info;
						save_avgs[1] = v;
					} // ul
					if (
						park_vertecs[0].distanceTo(v) <
						park_vertecs[0].distanceTo(save_avgs[0])
					) {
						poly_edges[0] = info;
						save_avgs[0] = v;
					} // ur
					if (
						park_vertecs[2].distanceTo(v) <
						park_vertecs[2].distanceTo(save_avgs[2])
					) {
						poly_edges[2] = info;
						save_avgs[2] = v;
					} // dr
					if (
						park_vertecs[3].distanceTo(v) <
						park_vertecs[3].distanceTo(save_avgs[3])
					) {
						poly_edges[3] = info;
						save_avgs[3] = v;
					} // dl
		...
		function s1(arr, al, op = 0) {
			// op 0 =x,1 = z
			if (op == 0) {
				return arr.slice().sort(function (a, b) {
					return a.x < b.x ? -1 * al : a.x > b.x ? 1 * al : 0;
				}); // x
			}

			return arr.slice().sort(function (a, b) {
				return a.z < b.z ? -1 * al : a.z > b.z ? 1 * al : 0;
			}); // z
		}
	...
	get_lines(
		...
		map1.forEach((val, key) => {
			...
			for (let i = 0; i < val.length; i++) {
				...
				if (!ind_to_vec.has(ind)) {
					ind_to_vec.set(
						ind,
						new THREE.Vector3(val[i].x, defualt_y, -val[i].y)
					);
				}
			}
		});
		...
		for (const [node, adj] of nodes.entries()) {
			...
			for (const [range, adj] of map_inrange.entries()) {
				...
				if (
					inrange_vec.distanceTo(node_vec) < merge_d &&
					!non_repeate.has(node)
				) {
					flag = false;
		...
		for (const [node1, adj1] of map_inrange.entries()) {
			for (const [node2, adj2] of map_inrange.entries()) {
				...
				if (new_map.get(node1).has(node2)) {
					continue;
				} // dont repeate twice
	...
	get_right(t1, t2) {
		...
		for (const n of adj_next) {
			if (n !== t1) {
				// dont take origin as adj

				let vec_n = this.ind_to_vec.get(n); // vec of n

				var temp_ang =
					Math.atan2(-vec_n.z - -vec_t2.z, vec_n.x - vec_t2.x) - origin_ang;
...
function place_objs_inarea(arr, y_s, enlarge = 1) {
	// arr y == 0
	objs_scene(scene);
...
function get_pos(trails, simplex = null, r_t = 5, len = 20, np, na) {
	...
	for (let q = 0; q < arr_points.length - 1; q++) {
		...
		let cnt =
			simplex != null
				? Math.floor(
						len +
							simplex.noise(t_noise, t_noise) * THREE.MathUtils.randInt(15, 17)
				  )
	...
	for (let i = 0; i < arr_points2.length + 1; i++) {
		let dc = arr_points2[i % arr_points2.length][0].distanceTo(
			arr_points[i % arr_points2.length][0]
		); // dist c
		let dn = arr_points2[(i + 1) % arr_points2.length][0].distanceTo(
