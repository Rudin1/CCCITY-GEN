...
import {mesh_containers, main_matetrial,get_color_b,extend_line,draw_line,y_objs,add_container} from './a_tools.js'
...
function init_objs(scene_instances){
            ...
            if(first_time){
                ...
                main_truck = new THREE.Mesh(new THREE.CylinderGeometry(r,r,h,8,1), main_mat ) ; 
...
function get_main_vent(){
	...
	.absarc( 0, 0, 0.7, 0, Math.PI * 2, true );
	...
	let m = new THREE.MeshPhongMaterial( { color: 0xffffff,side:THREE.DoubleSide} );
	...
	let m5 = new THREE.MeshPhongMaterial( { color: 0xffffff,side:THREE.DoubleSide} );
...
function get_tree(pos,color = 0,col_grp = true, size = [2,2] , amount = 65 , type = 1,fm = null,ea1 = null,ea2 = null){
    ...
    add_container(main_leaf,'cone','cone',r*2,ht,0,world_pos,world_quat,colt,1)  //*1.8
    ...
    if( ea1!=null && (Math.abs(pos.z-ea1[0].z) < e || Math.abs(pos.z-ea1[2].z) < e || Math.abs(pos.x-ea2[0].x) < e || Math.abs(pos.x-ea1[1].x) < e)){return;}
    ...
    if(scatter){
        ...
        for(let i=0;i<5;i++){
            ...
            add_container(main_leaf,'cone_scat','cone_scat',r1* 0.05,h_t* 0.05,0,world_pos,world_quat,col1,1,y_objs) 
...
function get_water_con(pos,y_s,scl,look = null){
       ...
       let t_col2 = window_cols[Math.floor((window_cols.length-0.01)*Math.random())] 
    let op_scl = 1/scl;
    ...
    let col2 = window_cols[Math.floor((window_cols.length-0.01)*Math.random())] 
...
function get_vent(pos,y_s,scl,look = null){
    ...
    if(look!==null)
	vent.lookAt(look) // floor == 0 == rotateY

    vent.position.y = pos.y + y_s;


    //same not as floor
    let t_col1 = window_cols[Math.floor((window_cols.length-0.01)*Math.random())] 
    let t_col2 = window_cols[Math.floor((window_cols.length-0.01)*Math.random())] 
    let t_col3 = window_cols[Math.floor((window_cols.length-0.01)*Math.random())] 
    ...
    // //    mesh_containers['box'][0].push(mesh)
       mesh_containers['vent_hole'][0].push(
        ...
        add_container(bt,'plane','plane',    1.4,1.4,0,world_pos,world_quat,t_col1,scl)

        let r = Math.random()*Math.PI*2
        vent.rotateY(r)

        add_container(p1,'plane','plane',     0.2,1.2,0,world_pos,world_quat,t_col2,scl)


    

        add_container(p2,'plane','plane',     0.2,1.2,0,world_pos,world_quat,t_col3,scl)
	...
	/// anttena/ tower / ifel
	function rec(temp,rad,y_s = 0,alpha = 2,d = 5,sides = 15,line_segs = 3,change = 0.9,flag_change = 0,center = new THREE.Vector3(),grid_line = null){
        ...
        if(flag_change == 0){
		rec(temp,rad*change,y_s + alpha,alpha,d-1,sides,line_segs,change,flag_change,center,grid_line)
}
        if(flag_change == 1){
        rec(temp,rad + change,y_s,alpha,d-1,sides,line_segs,change,flag_change,center,grid_line)}
...
function get_tower(pos,scl=1,y_s=0,radius = 5,alpha = 3,depth = 15,sides = 3,line_segs = 5,change = 1){
    ...
    function get_rect(pos,y_s,w,h,d,look = null){
        ...
        let arr = [ang*0.75,ang*0.50,ang*0.25]
