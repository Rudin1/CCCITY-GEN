...
export {flag_mouse,flag_cars,y_objs,chance_car,flag_windows,split_vertecs,flag_building,add_y,amt_tree,gl_w,gl_h}
...
export {len_h,len_v,s_v,e_v,s_h,e_h,h_n,v_n,pul,pur,pdl,pdr,edge_method,rad_edges,sh,eh,sv,ev,flag_create}
...
export {mesh_containers,draw_pencil,color_w,car_nums,defualt_y,arr3d_to_2d,scl_cars}
...
export {camera_pos,Precision,OffsetContour,main_matetrial,flag_instance,k_lines,n_lines,alp_lines}
...
function d3_shape(len1,len2,len3){
	...
	////// box
	let b = new THREE.BoxGeometry(len1-1,len2-1,len3-1,len1-1,len2-1,len3-1) // same size segs/len to get same res.
	...
	for(let i=0;i<pos.length;i+=3){
		...
		if(grid3d_vert[v.x][v.y][v.z]==null)
		grid3d_vert[v.x][v.y][v.z] = v
...
export {collide_point}
        ...
        function get_noise_memo(p1,ang1,arr, main_simplex = null,scene = null,np,na,aa = false){
...
function get_trail_noise2(scene,t1,t2,a1,a2,arr,simplex = null,np=22,ma = 6){
		...
		for(let k=0;k<2;k++){
				...
				while(!collide_point(arr,p1,1.5,scene)){ // 1
...
function get_trail_noise(scene,t1,t2,a1,a2,park,edges,gr,simplex = null,np=22,ma = 6){
	// console.log("gr=",gr)
	let s = scene;
	simplex = simplex== null ? new SimplexNoise():simplex;
	// get_sphere(t1.x,t1.y,t1.z,0x00ffff,1)
		...
		let c = 0
		let step = 0.01
		while(is_inrange(particles[0],park,edges,gr) || is_inrange(particles[1],park,edges,gr) ){
			// while(collide_point())
			c+=step
			// console.log("shti")
			if(i>cnt){
...
function add_container(obj,name,sub_name,w,h,d,p,q,col,scl,ya = 0){
	...
	let s = 
	name === 'box'  || name === 'win_box' ?  [w,h,d]: // 3
	name === 'win' || name === 'dbl_pln' ||  name === 'plane' || name === 'cylinder'  ||  name === 'cone'   || name === 'cone_scat'  ? [w,h] :// 2 // cylinder,cone,cone_r
	name === 'sphere' ? [d]: // 1
	[]; // 0 wheel,top,body
