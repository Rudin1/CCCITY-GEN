...
class VehicleGenerator{
    ...
    constructor(al,flag_firsttime = true,s = null,col = 0xffff00){ 
		...
		scene = s;
			...
			// wheels
			for(let i=0;i<arr_wheels.length;i++){	 // arr cars	
				
					for(let j=0;j<arr_wheels[i].length;j++){// arr wheel info pairs
					

						for(let z=0;z<3;z++){// arr wheel inside pairs
							arr_wheels[i][j][z]*=al;
	...
	update(car,v = null){ // car == group parts
		if(!v){
	...
	get_wheels(type){
			...
			let b = 0.9
			let gf1 = new THREE.CylinderGeometry(wheels[0][2]*b,wheels[0][2]*b,wheels[0][2]*0.5,15) //front
			let gf2 = new THREE.CylinderGeometry(wheels[0][2]*b,wheels[0][2]*b,wheels[0][2]*0.5,15) //front

			let gb1 = new THREE.CylinderGeometry(wheels[0][2]*b,wheels[0][2]*b,wheels[0][2]*0.5,15) // back
			let gb2 = new THREE.CylinderGeometry(wheels[0][2]*b,wheels[0][2]*b,wheels[0][2]*0.5,15) // back
			...
			wf1.position.set(wly[0]/2,wheels[1][1],wheels[1][0]-wly[1]/2 + rad-rad*rad_add) //// LITTLE GAP **** - rad_add???
			wf1.rotateZ(Math.PI/2)

			wf2.position.set(-wly[0]/2,wheels[1][1],wheels[1][0]-wly[1]/2 + rad-rad*rad_add) //// LITTLE GAP **** -rad*0.2 ???
			wf2.rotateZ(Math.PI/2)

			wb1.position.set(wly[0]/2,wheels[0][1],wheels[0][0]-wly[1]/2+ rad-rad*rad_add) //// LITTLE GAP **** -rad*0.2 ???
			wb1.rotateZ(Math.PI/2)


			wb2.position.set(-wly[0]/2,wheels[0][1],wheels[0][0]-wly[1]/2+ rad-rad*rad_add) //// LITTLE GAP **** -rad*0.2 ???
	...
	get_vehicle(type,col = 0xffffff){ // optimizeeeeeeeeeeee latter ******************
		let arr,body;
	...
	get_shape(n,col){ 
		...
		//this.col2
		let m = new THREE.MeshPhongMaterial( { color: c ,wireframe:false,side:THREE.DoubleSide} );
	...
	get_top2(grid,type,col){
		...
		let m_c = new THREE.MeshPhongMaterial({color:0x333333, wireframe:false})
		let m_w = new THREE.MeshPhongMaterial({color:0x333333, wireframe:false})
		...
		// beta[0] = -2.27
		// beta[1] = 1
		grid[0][0][0].forEach(ind=>{ // b,r
			// get_sphere(pos[ind],pos[ind+1],pos[pos+2])

			pos[ind+2] +=beta[0]*al ; 
		})

		grid[0][0][1].forEach(ind=>{// f,r
			// get_sphere(pos[ind],pos[ind+1],pos[pos+2])
		...
		let top_plane = new THREE.PlaneGeometry(brv.x -blv.x  + 10,frv.z - brv.z + 10) //brv.x -blv.x
...
let ava = 1.3
let arr_wheels = [
	[[3*0.3, 1*0.3, 0.5, 0, Math.PI * 1, false], [5.7, 1*0.3, 0.5, 0, Math.PI * 1, false]  ], // normal
	[[3*0.3, 1*0.3, 0.5, 0, Math.PI * 1, false], [5.8, 1*0.3, 0.5, 0, Math.PI * 1, false]  ], // long

	[[2.4, 0.6, 1, 0, Math.PI, false], [12.6, 0.6, 1, 0, Math.PI, false]  ], // bus  *


	[[3*0.4, 1*0.3, 0.5, 0, Math.PI * 1, false], [5.7, 1*0.3, 0.5, 0, Math.PI * 1, false]  ], // pickup
	[[3*0.4, 1*0.3, 0.5, 0, Math.PI * 1, false], [5.7, 1*0.3, 0.5, 0, Math.PI * 1, false]  ], // sedan1


	[[0.7, 1*0.3, 0.5, 0, Math.PI * 1, false], [3, 1*0.3, 0.5, 0, Math.PI * 1, false]  ], // micro

	[[1.5*ava, 1*0.3*ava, 0.5*ava, 0, Math.PI * 1, false], [5.5*ava, 1*0.3*ava, 0.5*ava, 0, Math.PI * 1, false]  ], //van
...
// add more types
let get_arr_type = [
[[0,0.3],[0,1],[1.6,1.1],[6,1.1],[7,1],[7,0.4]], //	 0 sedan
[[0,0.3],[0,1],[2,1.2],[3.0,1.2],[7,1.2],[7,0.4]], // 	1 wagon

[[0,0.6],[0,4],[16,4],[16,0.8],[14.0,0.6]], // 2 	bus (or instead last 2, [4,0])


[[0,0.3],[0,1.2],[1.6,1.3],[4,1.3],[4,1.0],[7,1.0],[7,0.3]], //	 3 pickup 
[[0,0.3],[0,1],[2,1.2],[3.0,1.2],[7,1.2],[7,0.4]], // 	4 hatchback


[[0,0.3],[0,1],[1.6,1.1],[4,1.1],[4,0.3]], // micro




[[0,0.3*ava],[0.2*ava,1.2*ava],[1.1*ava,1.45*ava],[3.7*ava,1.45*ava],[3.7*ava,2.5*ava],[7*ava,2.5*ava],[7*ava,0.3*ava]], //	 3 van/truck 
...
[
	...
	[-2.27,1,1, -2.27,0.3,-0.3,0.3,-0.3,-1.7,-1.7,0,0,0.3,0.3,0.3,0.3], // x
	[0.5,1,1,-2.27, 0.3,-0.3,0.3 ,-0.3,0.5,0.5,0 ,0 ,0.3,0.3,0.3,0.3] , 
	[-2.2,1,1,1.2,0.3,-0.3,0.3,-0.3,-1.1,-1.1,-0.5,-0.5, 0.1,  0.1,  0.1, 0.1 ], // 4
	[-0.51,0.53,0.53,-0.7,0.2,-0.2, 0.2,-0.2,-0.1, -0.1,-0.3,-0.3, 0.22,0.22,0.22,0.22], // micro // 5

	[0.5*ava,1*ava,1*ava,-2.27*ava, 0.3*ava,-0.3*ava,0.3*ava ,-0.3*ava,0.5*ava,0.5*ava,0 ,0 ,0.5*ava,0.5*ava,0.5*ava,0.5*ava] ,  // van
