...
let main_truck = new THREE.Mesh(
	new THREE.CylinderGeometry(2, 2, 2, 8, 2),
	main_mat
);
...
class overcross {
	...
	constructor(
		...
		arr_roads.forEach((road) => {
			...
			for (let i = 0; i <= n / 2; i++) {
				...
				if (i == 0) {
					arr_lanes[mid] = new THREE.Vector3(
						v.x + vector_l.x,
						v.y + vector_l.y,
						v.z + vector_l.z
					);
				} else {
					arr_lanes[mid - i] = new THREE.Vector3(
						v.x + vector_l.x,
						v.y + vector_l.y,
						v.z + vector_l.z
					);
	...
	straight2(
		...
		if (!parallel_lanes) {
			...
			let v_1l = new THREE.Vector3()
				.copy(dir_r1)
				.multiplyScalar(len_arrow)
				.add(r1_l);
			let v_1r = new THREE.Vector3()
				.copy(dir_r1)
				.multiplyScalar(len_arrow)
				.add(r1_r);
			let v_2l = new THREE.Vector3()
				.copy(dir_r2)
				.multiplyScalar(len_arrow)
				.add(r2_l);
			let v_2r = new THREE.Vector3()
				.copy(dir_r2)
				.multiplyScalar(len_arrow)
				.add(r2_r);
	...
	co_connect(graph, ra_e, ra_in, flag_switch, edge_mon, in_mon) {
		...
		if (op == 1) {
			...
			let op1 =
				option == 2
					? random_pick([0, 1, 2, 3]) // pick tree or mons
					: option == 1
					? random_pick([1, 2, 3]) // mon
					: option == 0
					? 0 // tree
					: 1000; // nothing.
			...
			if (op1 == 1) {
				...
				for (let j = 0; j < 4; j++) {
					...
					const r2 =
						Math.sqrt(2 * THREE.MathUtils.randFloat(0, 0.5 * rt2 * rt2)) * 0.4;
					const th2 = THREE.MathUtils.randFloat(0, Math.PI * 2);
					const dx = r2 * Math.sin(th2);
					const dz = r2 * Math.cos(th2);
		...
		for (let p = 0; p < phase.con.length; p++) {
			...
			if (Math.abs(ang2 - ang1) < 0.1) {
				// connect by lanes striaght.
				flag_parallels = true;
			}

			if (type_kind === "ls") {
				// large,small junction,
				...
				if (phase.con[p][2] == "half_first") {
					let flag_ls = true; // l-->
					if (arr[t1][6].length > arr[t2][6].length) {
						// l --> s
	...
	quad(n1, n2, r1, r2, i, par = false, forward = null, rad = 1) {
		// n's == lanes of lane cars
		...
		let r1_v =
			arr[r1][arr[r1].length - 1][n1 % arr[r1][arr[r1].length - 1].length]; // l of lane r1
		let r2_v =
			arr[r2][arr[r2].length - 1][n2 % arr[r2][arr[r2].length - 1].length]; // r of lane r1
		let dir_r1 = arr[r1][5];
		let dir_r2 = arr[r2][5];
		...
		if (!par) {
			...
			curve = new THREE.CubicBezierCurve3(r1_v, p1, p2, r2_v);
			...
			//par
			curve = new THREE.LineCurve3(r1_v, r2_v);
		}
		...
		get_cars_road_memo(
			...
			null,
			true
		); // * combine with _test latter *
	}

	quad2(n1, n2, r1, r2, i, par = false, forward = null) {
		// n's == lanes of lane cars
		...
		if (!par) {
			...
			// find intersection straight
			let v_1 = new THREE.Vector3()
				.copy(dir_r1)
				.multiplyScalar(len_arrow)
				.add(r1_v);
			let v_2 = new THREE.Vector3()
				.copy(dir_r2)
				.multiplyScalar(len_arrow)
				.add(r2_v);
			...
			curve = new THREE.QuadraticBezierCurve3(r1_v, res, r2_v);
		} else {
			//par
			curve = new THREE.LineCurve3(r1_v, r2_v);
		}
...
function get_cars_road_test(
	...
	// choose lane
	vec1.set(
		g.vertices[n_road * c * aa + 0],
		g.vertices[n_road * c * aa + 1],
		g.vertices[n_road * c * aa + 2]
	);
	vec2.set(
		g.vertices[n_road * c * aa + wss * c + 0],
		g.vertices[n_road * c * aa + wss * c + 1],
		g.vertices[n_road * c * aa + wss * c + 2]
	);
	var dir = new THREE.Vector3();
	...
	vec3.set(
		g.vertices[n_road * c + 0],
		g.vertices[n_road * c + 1],
		g.vertices[n_road * c + 2]
	);

	// loop over selected road, put on front if no intersection
	for (let s = n_road * c + t1; s < g.vertices.length - t2; s += wss * c) {
		///*** */ optimize latter === add noise sides + front

		let vec1 = new THREE.Vector3();
		let vec2 = new THREE.Vector3();
		vec1.set(g.vertices[s + 0], g.vertices[s + 1], g.vertices[s + 2]);
		vec2.set(
			g.vertices[s + wss * c + 0],
			g.vertices[s + wss * c + 1],
			g.vertices[s + wss * c + 2]
		);
		...
		if (qq && cur_dist > sum_limit) {
			let res =
				memo === null ? true : check_collision(vec1, c_car, r, dir, memo);
			if (res !== null) {
				// console.log("res",res)
				...
				sum_limit =
					(get_wly[prev_kind][1] * 0.58 + get_wly[r][1] * 0.58) * alpha; ////  scale of car // 0.5
			}
		} //if
...
function get_car(
	...
	car_group.position.set(pos.x, pos.y, pos.z);
	...
	let sub_name = type == 2 ? "bus" : type == 5 ? "micro" : "car";
...
function get_cars_road2(g, n_road = 1, n_lane = null) {
	...
	let arr_curve = [];
	for (
		let s = n_road * 3, n1_s = n1 * 3, n2_s = n2 * 3;
		s < g.vertices.length - 1;
		s += wss * 3, n1_s += wss * 3, n2_s += wss * 3
	) {
		///*** */ optimize latter === add noise sides + front
		...
		if (n_lane !== null) {
			...
			let n_v1 = new THREE.Vector3(
				g.vertices[n1_s + 0],
				g.vertices[n1_s + 1],
				g.vertices[n1_s + 2]
			);
			let n_v2 = new THREE.Vector3(
				g.vertices[n2_s + 0],
				g.vertices[n2_s + 1],
				g.vertices[n2_s + 2]
			);
...
function get_road_strips(g, n_road, num_lanes, alpha, shape) {
	...
	for (let s = step; s + 3 < arr_points.length - step; s += step) {
		main_plain.position.set(
			arr_points[s + 2].x,
			arr_points[s + 2].y,
			arr_points[s + 2].z
		); // middle of 2 vectors, add epsilote to avoid flickerring
	...
	if (arr_points.length - 5 > 0) {
		for (let i = 0; i < 2; i++) {
			main_plain.position.set(
				arr_points[pos[i] + 2].x,
				arr_points[pos[i] + 2].y,
				arr_points[pos[i] + 2].z
			); // middle of 2 vectors, add epsilote to avoid flickerring
...
function check_collision(v, c_car, type, dir, memo) {
	...
	for (let i = 0; i < memo.length; i++) {
		...
		for (let j = 0; j < points.length; j++) {
			if (memo[i].distanceTo(points[j]) <= radious * 1 * alpha) {
				// 2 bcz 2 raduis of 2 circles dist
				return null;
			}
		}
	}
